$(document).ready(function(){
    ...
    let functions = (function(){
        let validateFields = function(){
            ...
            if(check){
                if(!postalCodeRegex.test($(".form-wrapper .form input[name='postalCode']").val())){
                    check = false;
                    alert("Postal Code (Acceptable format: NAN ANA – where 'A' is any alphabetic character and 'N' is avalid numeric digit)");
                }
                if(!phoneNumberRegex.test($(".form-wrapper .form input[name='phoneNumber']").val())){
                    check = false;
